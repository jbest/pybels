...
def find_potential_duplicates_orig(df, similarity_threshold, method='fuzzy', county_name=None):
    i = 0
    j = 0
    groups = []
    group_id = 1
    ...
    for i, row1 in df.iterrows():
        ...
        print('county:', county_name, 'i:', i, 'assigned_groups len:', len(assigned_groups))
        ...
        for j, row2 in df.iterrows():
            print('county:', county_name, 'df len:', len(df), 'i:', i, 'j:', j, 'assigned_groups len:', len(assigned_groups))
...
def save_filtered_groups_to_csv_ORIG(input_filename, df, group_assignments, sub_group_assignments, min_size, export_columns, allowed_institutions):
...
def main():
    ...
    if csv_file:
        ...
        for county in counties:
            ...
            #print(bels_location_id)
            #make copy of dataframe
            #df_copy = df.copy()
            #df_bels_county = df_copy[df_copy['county'] == county]
            df_bels_county = df[df['county'] == county]
            print(county, df_bels_county.shape)
            #print(bels_location_id, df_bels_group['locality'])
            #df_bels_rep = df_bels_group.groupby('bels_location_id').first()
            #print(df_bels_rep['bels_location_id'])
            #print(str(df_bels_rep['id'].values[0]))
            ...
            similarity_threshold = 95
            groups, group_assignments, last_group_id = find_potential_duplicates(df_bels_county, similarity_threshold, method='fuzzy', county_name = county)
            ...
            filename = county + '_BELS_Grouper.csv'
            df_bels_county.to_csv(filename, sep='\t')
